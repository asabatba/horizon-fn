...
import { add, clamp, dot, exp, len, norm, scale, type Vec3 } from "./utils.js";
...
// Night sky ambient (airglow, diffuse starlight) — prevents pure black at night.
// Applied with base EXPOSURE only (not the twilight boost) so deep night stays dark.
// Tuned so that EXPOSURE × NIGHT_AMBIENT → ACES → gamma lands near rgb(5,8,20).
const NIGHT_AMBIENT: Vec3 = [6e-5, 1.5e-4, 9e-4];
...
function computeTransmittance(height: number, angle: number) {
	...
	// If the ray toward the sun hits the ground, no light gets through
	const groundHit = intersectSphere(rayOrigin, rayDirection, GROUND_RADIUS);
	if (groundHit !== null && groundHit > 0) return [0, 0, 0];
...
export default function renderGradient(altitude: number): [string, Vec3, Vec3] {
	...
	// Eye adapts as the sun goes below the horizon. Continuous at 0° (exp(0)=1),
	// ramps up to 15× by ~-12° to reveal twilight/night atmospheric scattering.
	const exposureScale =
		altitude < 0 ? Math.min(Math.exp(-altitude * 20), 15) : 1;
	const effectiveExposure = EXPOSURE * exposureScale;
	...
	for (let i = 0; i < SAMPLES; i++) {
		...
		// Scattered light uses the twilight-boosted exposure; ambient uses only the base
		// exposure so deep night stays dark regardless of how much the boost ramps up.
		let color: Vec3 = [
			inscattered[0] * effectiveExposure + NIGHT_AMBIENT[0] * EXPOSURE,
			inscattered[1] * effectiveExposure + NIGHT_AMBIENT[1] * EXPOSURE,
			inscattered[2] * effectiveExposure + NIGHT_AMBIENT[2] * EXPOSURE,
		];
		color = applySunsetBias(color);
		color = aces(color);
		color = color.map((c) => c ** (1.0 / GAMMA)) as Vec3;
